...
+ (NSArray<DTRoutingTableItem *> *)getRoutingTable {
    @try {
        // Set the router array variable with the routing information
        NSMutableArray *routerArray = [DTRouter getRoutes];
        NSMutableArray *res = [NSMutableArray new];
        
        for(int i = 0; i < (int)[routerArray count]; i++)
        {
            // Set the router info
            DTRouter* router = (DTRouter*)[routerArray objectAtIndex:i];
            DTRoutingTableItem *item = [[DTRoutingTableItem alloc] init];

            item.destination = [DTRouter getStdFormatSting:[router getDestination]];
            item.gateway = [DTRouter getStdFormatSting:[router getGateway]];
            item.flags = [DTRouter getStdFormatSting:[router getFlags]];
            item.refs = [DTRouter getStdFormatSting:[router getRefs]];
            item.use = [DTRouter getStdFormatSting:[router getUse]];
            item.mtu = [DTRouter getStdFormatSting:[router getMTU]];
            item.expire = [DTRouter getStdFormatSting:[router getExpire]];
            item.netif = [DTRouter getStdFormatSting:[router getNetif]];
...
+ (NSString *)getRoutingDesc
{
    __block NSString *showStr = @"Destination        Gateway            Flags        Refs      Use   Netif Expire    \r\n";
    
    NSArray<DTRoutingTableItem *> *result = [DTRouter getRoutingTable];
    [result enumerateObjectsUsingBlock:^(DTRoutingTableItem *obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
+ (nullable DTRouter*) getRoute:(struct rt_msghdr2 *)rtm
{
    ...
    if(rtm->rtm_addrs & RTA_DST)
    {
        ...
        {
            route = [[DTRouter alloc] initWithRtm:rtm];
        }
    }
    return route;
}
...
@implementation  DTRoutingTableItem
-(NSString *)desc{
